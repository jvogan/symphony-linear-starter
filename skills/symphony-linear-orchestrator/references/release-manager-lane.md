...
The job authenticates `gh` with the default `GITHUB_TOKEN` (needs `contents: write` and `pull-requests: write`, both set in the sample). Events raised by `GITHUB_TOKEN` do not cascade to other workflows, but the merge-queue build is generated by the queue itself, so CI still runs. If you need the enqueue *itself* to trigger other workflows, use a PAT secret instead.
