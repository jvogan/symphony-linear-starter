...
> **Heads-up:** if `In Review` is in the lane's `ready_states` (the rendered default), enabling the lane makes a `release:ready` issue in `In Review` auto-enqueue — so `In Review` stops being a human gate and becomes an auto-merge trigger. To keep a human review gate, put only `Ready to Merge` in `ready_states` and have workers stop at `In Review` until you promote them.
...
### Workers must open the PR

The lane only acts on issues that already have a PR — nothing opens it for you. An implementation worker must push its branch and open a PR after validation passes, then record the URL in the outcome comment above. A minimal worker step:

```bash
git push -u origin "$(git branch --show-current)"
gh pr create --fill --base main   # prints the PR URL for the outcome comment
```

For a worker to run `gh pr create`, its sandbox needs two things the rendered default does **not** grant:

- **A PR-scoped GitHub token in the worker's env allowlist, added by name (never inlined).** The rendered worker command allows only `LINEAR_API_KEY` (`shell_environment_policy.include_only=["LINEAR_API_KEY"]`); add your token's variable, e.g. `include_only=["LINEAR_API_KEY","GH_TOKEN"]`. Use a fine-grained token scoped to PR write on this repo only.
- **Network egress** to GitHub from the worker sandbox.

Enable these **only** when you adopt the release flow — they broaden what every worker can do, so keep the token least-privilege and the change explicit. Workers still must never push to, rebase, or merge the base branch (see Roles); opening a PR from their own feature branch is the only Git write they perform.
...
5. Extracts the newest PR URL from `<!-- symphony-outcome -->`, comments, **Linear PR attachments, or the `owner/repo#N` shorthand** — so it still finds the PR when a connected Linear rewrites a pasted URL into a `linear.app/review` link (which carries no `github.com` URL).
...
**And enqueue with a PAT, not `GITHUB_TOKEN`.** That `merge_group` build is itself a workflow run, and events from `GITHUB_TOKEN` do not cascade — so a PR enqueued with `GITHUB_TOKEN` never triggers it and sits in the queue until it times out. The lane's scheduled Action and the github-native sample read `secrets.RELEASE_PAT` / `secrets.AUTOMERGE_PAT` and fall back to `GITHUB_TOKEN` only when unset; set the PAT whenever the queue has required `merge_group` checks.
...
### Run it hands-off (scheduled GitHub Action)
...
# 2. move the sample into place and commit it
mkdir -p .github/workflows
cp .orchestration/release-manager.gha.yml .github/workflows/release-manager.yml
```

Set the cadence at bootstrap time with `--release-cron` (default `*/10 * * * *`), or edit the `schedule:` in the file.

The one non-obvious requirement is the `concurrency:` block, and it is load-bearing: **a GitHub Actions runner is a fresh host every run, so the lane's per-host file lock cannot stop two overlapping runs from both writing to the base branch.** The concurrency group (`release-manager-<branch>`, `cancel-in-progress: false`) is the cross-run lock that keeps the lane single-writer — one run executes at a time, an in-flight pass is never killed mid-write, and a run that arrives while one is active queues behind it. Do not remove it.

The job needs `contents: write` and `pull-requests: write` (both set in the sample). **Token choice is load-bearing with a merge queue:** events raised by `GITHUB_TOKEN` do not cascade, so a `GITHUB_TOKEN` enqueue does **not** trigger the queue's `merge_group` CI — queued PRs then never merge. Set a `RELEASE_PAT` secret (a repo or fine-grained PAT with contents + pull-requests write) and the sample uses it automatically (`secrets.RELEASE_PAT || secrets.GITHUB_TOKEN`). `GITHUB_TOKEN` alone is fine only in the no-queue serial fallback, where there is no `merge_group` CI to trigger.

This is a **poll** (cron), not a push: the Action re-runs the lane and the lane reads Linear each pass. For sub-minute landing, drive the same `release_manager.py --apply` command from a Linear webhook instead — the lane is idempotent, so any trigger cadence is safe.

Two operational notes for a long-lived hands-off loop: (1) the queue only completes if your CI also triggers on `merge_group` events (see "Make CI run for the queue") — otherwise queued PRs never finish and every drain pass burns to its `max_passes` cap. (2) GitHub **auto-disables a scheduled workflow after ~60 days of repository inactivity**; on a quiet repo, re-enable it (or keep an unrelated heartbeat) so the loop keeps firing.
...
- GitHub Merge Queue needs a ruleset and CI wired to `merge_group` events (see "Verify and enable" above). The lane verifies the queue is *enabled* and warns — or blocks, with `require_merge_queue: true` — when it is missing; it does **not** check that your CI triggers on `merge_group`, so verify that yourself or queued PRs never finish.
